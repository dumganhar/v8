...
#ifndef V8_HEAP_CPPGC_INTERNAL_HEAP_OBJECT_HEADER_H_
#define V8_HEAP_CPPGC_INTERNAL_HEAP_OBJECT_HEADER_H_
...
HeapObjectHeader::HeapObjectHeader(size_t size, GCInfoIndex gc_info_index) {
  ...
  v8::base::AtomicRef<uint16_t>(encoded_high_)
...
bool HeapObjectHeader::TryMarkAtomic() {
  v8::base::AtomicRef<uint16_t> atomic_encoded(encoded_low_);
...
uint16_t HeapObjectHeader::LoadEncoded() const {
  ...
  return v8::base::AtomicRef<uint16_t>(const_cast<uint16_t&>(half))
      .load(memory_order);
}
...
void HeapObjectHeader::StoreEncoded(uint16_t bits, uint16_t mask) {
  ...
  v8::base::AtomicRef<uint16_t> atomic_encoded(half);
...
#endif  // V8_HEAP_CPPGC_INTERNAL_HEAP_OBJECT_HEADER_H_
